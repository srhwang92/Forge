...
Don't run mid-phase (state will be stale on day one). If `CONTEXT.md` or project `CLAUDE.md` doesn't exist, stop — they're the inputs.

## Read

Project `CLAUDE.md`, `CONTEXT.md`, `PHASE.md`, latest 3 logs, `GUARDRAILS.md` (if exists), top-level `README.md`, `package.json` (or equivalent), `tests/canaries/` listing.
...
1. **Architecture Overview** — 1-paragraph from `CONTEXT.md` Architecture; text diagram if 3+ moving pieces.
...
4. **Product Context** — decisions from `CONTEXT.md` Key Decisions that influenced technical choices but aren't obvious from source.
...
7. **Forge Framework Notes** — brief explanation of `CLAUDE.md` / `CONTEXT.md` / `tests/canaries/` for devs who don't use Claude Code.
